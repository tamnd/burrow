...
`burrow/math/rand/v2.h` is Go's `math/rand/v2` and `burrow/math/rand.h` is the older `math/rand`. Both give the same numbers Go does: the same seed gives the same sequence, method for method, and the tests check that against the tables Go keeps for exactly that purpose.

For most uses the package level functions are all you need. They draw from a generator seeded by the system, with its state kept per thread, so they are safe from any number of threads and never contend over a lock:

<!-- example: ../examples/numbers/rand.c#global -->
```c
Int die = mathrand2_int_n(6) + 1; /* 1 to 6, different every run */
double f = mathrand2_float64();   /* in [0, 1) */
```

A program that wants the same sequence every run makes a generator of its own from a seeded source. `Mathrand2PCG` is the small fast one and `Mathrand2ChaCha8` is the one to pick when the output should also be hard to predict. Go's `Rand` is `Mathrand2Rand` here, because `mathrand2_int_n` and the rest are already the package level functions, and its methods are `mathrand2_rand_int_n` and so on:

<!-- example: ../examples/numbers/rand.c#seeded -->
```c
...
Those are `76 61 78 79 23` on every run and every machine, which is what `rand.New(rand.NewPCG(1, 2))` gives in Go. A generator made this way is not safe to share between threads without a lock, the same as in Go.

`Shuffle` takes a `SwapFunc`, which is a function and a pointer for it to use, and calls it for each pair it swaps. `swap_cards` here swaps two entries of the array it is given:

<!-- example: ../examples/numbers/rand.c#shuffle -->
```c
mathrand2_rand_shuffle(r, 5, BURROW_FN(SwapFunc, swap_cards, deck));
```

`N` is generic in Go, and here `mathrand2_n` and `mathrand2_rand_n` pick the signed or unsigned version from the type of their argument, so `mathrand2_n((uint8_t)3)` works the way `rand.N[uint8](3)` does.

The first version is here for code written against it, and for the sequences a seeded `rand.NewSource` gives, which programs and their tests have come to depend on. Its `Rand` is `MathRandRand`, so that its methods, `math_rand_rand_intn` and the rest, do not collide with the package level functions:

<!-- example: ../examples/numbers/rand.c#v1 -->
```c
...
Since Go 1.24 the package level `Seed` does nothing, and `math_rand_seed` is the same. `GODEBUG=randseednop=0` brings the old behaviour back and `GODEBUG=randautoseed=0` starts the shared generator seeded with 1, both read from the environment the first time they matter, as Go reads them.

ChaCha8 gives the same stream as Go's for the same 32 byte seed, and its state marshals to the same bytes, so a generator saved by a Go program can be picked up by a C one and carry on where it left off:

<!-- example: ../examples/numbers/rand.c#chacha -->
```c
Byte seed[32];
...
None of these are for keys, tokens or anything else that has to be hard to guess by someone trying. That is `crypto/rand`.
...
- `include/burrow/math/rand/v2.h` and `include/burrow/math/rand.h`, and `tests/rand_test.c`, which checks both against the tables in Go's own regression, PCG and ChaCha8 tests, read out by `tools/gen-rand-tests.sh`
