...
TEST(a_fresh_set_has_nothing_in_it) {
    burrow__Timers ts;
...
TEST(a_timer_that_is_due_runs) {
    burrow__Timers ts;
    burrow__Timer t;
    burrow__timers_init(&ts);
    burrow__timer_init(&t, note_fire, &t);
    forget_fires();

    bool pending = true;
    CHECK(burrow__timer_reset_on(&ts, &t, 1000, 0, NULL, NULL, &pending));
    CHECK(!pending);
    ...
    CHECK_INT_EQ(burrow__timers_wake_time(&ts), 0);
    CHECK_INT_EQ(t.state, 0);
    CHECK(t.ts == NULL);
    CHECK(!burrow__timer_stop(&t));

    burrow__timers_free(&ts);
}

TEST(a_timer_that_is_not_due_yet_says_when_it_will_be) {
    burrow__Timers ts;
    burrow__Timer t;
    burrow__timers_init(&ts);
    burrow__timer_init(&t, note_fire, &t);
    ...
    CHECK(burrow__timer_reset_on(&ts, &t, 5000, 0, NULL, NULL, NULL));
...
TEST(taking_a_reading_is_left_to_the_caller) {
    burrow__Timers ts;
    burrow__Timer t;
    burrow__timers_init(&ts);
    burrow__timer_init(&t, note_fire, &t);
    ...
    CHECK(burrow__timer_reset_on(&ts, &t, before + 1, 0, NULL, NULL, NULL));
...
TEST(timers_run_in_the_order_they_are_due) {
...
TEST(a_timer_moved_earlier_moves_the_wake_time_with_it) {
    burrow__Timers ts;
...
TEST(a_timer_moved_later_stays_where_it_was_until_somebody_looks) {
    burrow__Timers ts;
    burrow__Timer a, b;
    burrow__timers_init(&ts);
...
TEST(a_stopped_timer_stays_in_the_heap_and_does_not_fire) {
    burrow__Timers ts;
...
TEST(starting_a_stopped_timer_again_never_touches_the_heap) {
    burrow__Timers ts;
    burrow__Timer t;
    burrow__timers_init(&ts);
    burrow__timer_init(&t, note_fire, &t);
    ...
    CHECK(burrow__timer_reset_on(&ts, &t, 1000, 0, NULL, NULL, NULL));
    CHECK(burrow__timer_stop(&t));
    CHECK_INT_EQ(ts.zombies, 1);
    ...
    CHECK(burrow__timer_reset_on(&ts, &t, 4000, 0, NULL, NULL, NULL));
    CHECK_INT_EQ(ts.len, 1);
    CHECK_INT_EQ(ts.zombies, 0);
    CHECK_INT_EQ(t.state, BURROW__TIMER_HEAPED | BURROW__TIMER_MODIFIED);
...
TEST(a_set_that_is_mostly_stopped_timers_gets_cleaned_out) {
...
TEST(a_repeating_timer_arms_itself_again) {
    burrow__Timers ts;
    burrow__Timer t;
    burrow__timers_init(&ts);
    burrow__timer_init(&t, note_fire, &t);
    ...
    CHECK(burrow__timer_reset_on(&ts, &t, 1000, 1000, NULL, NULL, NULL));
...
TEST(one_set_can_take_over_another) {
...
TEST(freeing_a_set_disarms_what_is_left_in_it) {
    ...
    CHECK(burrow__timer_reset_on(&first, &t, 1000, 0, NULL, NULL, NULL));
    ...
    CHECK(burrow__timer_reset_on(&second, &t, 1000, 0, NULL, NULL, NULL));
    bool ran = false;
...
TEST(a_timer_wakes_a_thread_that_has_nothing_else_to_do) {
    wait_gate = (Gate){0};
    armed = false;
    waited = 0;
...
TEST(a_timer_on_a_busy_p_is_run_by_another_thread) {
    armed = false;
    waited = 0;
    fired_flag = 0;
...
int main(void) {
    RUN(a_fresh_set_has_nothing_in_it);
    RUN(a_timer_that_is_due_runs);
    RUN(a_timer_that_is_not_due_yet_says_when_it_will_be);
    RUN(taking_a_reading_is_left_to_the_caller);
    RUN(timers_run_in_the_order_they_are_due);
    RUN(a_timer_moved_earlier_moves_the_wake_time_with_it);
    RUN(a_timer_moved_later_stays_where_it_was_until_somebody_looks);
    RUN(a_stopped_timer_stays_in_the_heap_and_does_not_fire);
    RUN(starting_a_stopped_timer_again_never_touches_the_heap);
    RUN(a_set_that_is_mostly_stopped_timers_gets_cleaned_out);
    RUN(a_repeating_timer_arms_itself_again);
    RUN(one_set_can_take_over_another);
    RUN(freeing_a_set_disarms_what_is_left_in_it);
    RUN(a_timer_wakes_a_thread_that_has_nothing_else_to_do);
    RUN(a_timer_on_a_busy_p_is_run_by_another_thread);
    return harness_report("timer");
}
