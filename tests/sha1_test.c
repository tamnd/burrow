/* Derived from Go's src/crypto/sha1/sha1_test.go.
 * Go source: go1.27.1.
 *
 ...
 * value and never allocate, and Sum allocates only what it appends. The ConstantTimeSum pass of TestGolden goes with
 * it, since that method is not exported.
 *
...
static const Golden golden[] = {
    {"76245dbf96f661bd221046197ab8b9f063f11bad",
     ...
     "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
     "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
     "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\x0a"},
    ...
    {"74c51fa9a04eadc8c1bbeaa7fc442f834b90a00a",
     "The major problem is with sendmail.  -Mark Horton"},
    {"0b4c4ce5f52c3ad2821852a8dc00217fa18b8b66",
     "Give me a rock, paper and scissors and I will move the world.  CCFestoon"},
    {"3ae7937dd790315beb0f48330e8642237c61550a",
...
static Int sum(Slice in, Byte *out) {
    Sha1SumRet r = sha1_sum(in);
    memcpy(out, r.a, sizeof r.a);
    return (Int)sizeof r.a;
}

static void TestGolden(TestingT *t) {
    testdigest_golden(t, "sha1", sum, sha1_new, golden, NGOLD(golden));
}

static void TestSize(TestingT *t) {
    testdigest_sizes(t, "New", sha1_new, SHA1_SIZE, SHA1_BLOCK_SIZE);
}

static void TestSHA1Hash(TestingT *t) {
    testhash_without_clone(t, sha1_new);
}

static void BenchmarkHash8Bytes(TestingB *b) {
    testdigest_bench(b, sha1_new, 8);
}

static void BenchmarkHash1K(TestingB *b) {
    testdigest_bench(b, sha1_new, 1024);
}

static void BenchmarkHash8K(TestingB *b) {
    testdigest_bench(b, sha1_new, 8192);
}
