 ...
 *
 *     const Type *t = type_by_name(BURROW_S("image.Point"), NULL);
 *
 ...
 *
 * reflect here is complete over types that were declared with BURROW_STRUCT,
 * generated by burrow-gen, or written out by hand. It cannot see an arbitrary
 * C struct nobody described, because that description does not exist in a C
 * binary, and it will not pretend otherwise.
 *
 * Asking about a value you are holding cannot hit that wall. An Any carries a
 * descriptor, so a type with none is a link error at the place the Any was
 * built rather than an answer that turns out to be wrong inside json_marshal.
 *
 * Asking by name can hit it, because the name usually came from outside the
 * program, so type_by_name and type_register say what went wrong instead of
 * leaving a caller to work it out from a NULL. Each is a sentinel: static,
 * allocation free, and compared with errors_is like any other.
 *
 * These are the whole list. A reflect function that cannot answer returns one
 * of them, and it returns one of them rather than its best guess. */

/* Nothing is registered under that name. Distinct from a bad name on purpose:
 * this one usually means a program at the other end of a connection knows a
 * type this program was not built with, which is a deployment to look at, and
 * not a bug in the caller's code. */
extern const Error type_err_not_registered;

/* The query is not a type name at all. Empty, or nothing but dots, or a
 * trailing dot with no name after it. A decoder handed a length prefixed
 * string that turned out to be empty lands here, and it is worth telling apart
 * from a name that was well formed and simply unknown. */
extern const Error type_err_name_invalid;

/* A different type is already registered under that name, and it stays
 * registered. Almost always two packages whose last path element is the same,
 * which is a mistake in the program rather than in its input. */
extern const Error type_err_conflict;
 ...
 *
 * err is type_err_not_registered or type_err_name_invalid whenever the result
 * is NULL, and is left as no error otherwise. It may be NULL, like every other
 * out parameter in the library, for a caller that only wants to know whether
 * there is a type. */
 ...
 * already was and to the same type. Go panics on a conflict; burrow returns the
 * answer and lets the caller decide, because the caller is usually a plugin
 * loader that has somewhere to put it.
 *
 * False comes with type_err_conflict when a different type already holds the
 * name, and the first one stays: a registry that let the second win would
 * change what a name means underneath whoever is already using it. It comes
 * with type_err_name_invalid for a descriptor with no name, since a nameless
 * type cannot be looked up and registering it would be a silent no-op. It comes
 * with burrow_err_out_of_memory when the table could not grow, which is the one
 * of the three that is worth retrying. */
 ...
 *
 * A name that does not fit gives the empty string rather than as much of itself
 * as there was room for. A truncated qualified name is still a well formed
 * qualified name, so it would go back through type_by_name and come out as
 * nothing, or worse as something, and the caller would never learn which. 256
 * bytes holds every name in Go's standard library with room to spare, so this
 * is a buffer to size correctly once rather than a case to handle. */
