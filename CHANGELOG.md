...
- `burrow/context.h`, which is Go's `context` package: cancellation, deadlines and request scoped values. `context_background`, `context_todo`, `context_with_cancel`, `context_with_deadline`, `context_with_timeout`, `context_with_value`, the four interface methods as `context_deadline`, `context_done`, `context_err` and `context_value`, the `context_canceled` and `context_deadline_exceeded` sentinels, and `context_free`. `docs/guides/context.md` is the guide.
