/* Derived from Go's src/crypto/md5/md5_test.go.
 * Go source: go1.27.1.
 *
 ...
 * value and never allocate, and Sum allocates only what it appends. TestBlockGeneric compares the assembly block
 * function against the generic one, and there is only the generic one here.
 *
...
static Int sum(Slice in, Byte *out) {
    Md5SumRet r = md5_sum(in);
    memcpy(out, r.a, sizeof r.a);
    return (Int)sizeof r.a;
}

static void TestGolden(TestingT *t) {
    testdigest_golden(t, "md5", sum, md5_new, golden, NGOLD(golden));
}

static void TestLarge(TestingT *t) {
    /* md5sum of "0123456789" * 1000 */
    testdigest_large(t, "md5 TestLarge", md5_new, 10000,
                     "2bb571599a4180e1d542f76904adc3df");
}

static void TestExtraLarge(TestingT *t) {
    /* md5sum of "0123456789" * 10000 */
    testdigest_large(t, "md5 TestExtraLarge", md5_new, 100000,
                     "13572e9e296cff52b79c52148313c3a5");
}

static void TestSize(TestingT *t) {
    testdigest_sizes(t, "New", md5_new, MD5_SIZE, MD5_BLOCK_SIZE);
}

static void TestMD5Hash(TestingT *t) {
    testhash_without_clone(t, md5_new);
}

static void BenchmarkHash8Bytes(TestingB *b) {
    testdigest_bench(b, md5_new, 8);
}

static void BenchmarkHash1K(TestingB *b) {
    testdigest_bench(b, md5_new, 1024);
}

static void BenchmarkHash8K(TestingB *b) {
    testdigest_bench(b, md5_new, 8192);
}
