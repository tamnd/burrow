...
/* A macro and not only a variable, because MSVC will not take a const
 * variable as an initializer inside another array. */
#define BIGTEST                                                                        \
    TP("Man is distinguished, not only by his reason, but by this singular "           \
       ...
       "O<DJ+*.@<*K0@<6L(Df-\\0Ec5e;DffZ(EZee.Bl.9pF\"AGXBPCsi+DGm>@3BB/F*&OCAfu2/"    \
       "AKY\n"                                                                         \
       "i(DIb:@FD,*)+C]U=@3BN#EcYf8ATD3s@q?d$AftVqCh[NqF<G:8+EV:.+Cf>-FD5W8ARlolDIa\n" \
       "l(DId<j@<?3r@:F%a+D58'ATD4$Bl@l3De:,-DJs`8ARoFb/0JMK@qB4^F!,R<AKZ&-DfTqBG%G\n" \
       ">uD.RTpAKYo'+CT/5+Cei#DII?(E,9)oF*2M7/c\n")

static const TestPair bigtest = BIGTEST;
