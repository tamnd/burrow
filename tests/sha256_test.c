/* Derived from Go's src/crypto/sha256/sha256_test.go.
 * Go source: go1.27.1.
 *
 ...
 * value and never allocate, and Sum allocates only what it appends. TestCgo is about the Go garbage collector.
 *
...
static const Golden golden224[] = {
    ...
    {"0f10c2eb436251f777fbbd125e260d36aecf180411726c7c885f599a",
     "He who has a shady past knows that nice guys finish last."},
    {"4d1842104919f314cad8a3cd20b3cba7e8ed3e7abed62b57441358f6",
     "I wouldn't marry him with a ten foot pole."},
    ...
    {"43fb8aeed8a833175c9295c1165415f98c866ef08a4922959d673507",
     "Even if I could be Shakespeare, I think I should still choose to be Faraday. - "
     "A. Huxley"},
    {"ec18e66e93afc4fb1604bc2baedbfd20b44c43d76e65c0996d7851c6",
     "The fugacity of a constituent in a mixture of gases at a given temperature is "
     "proportional to its mole fraction.  Lewis-Randall Rule"},
...
static Int sum256(Slice in, Byte *out) {
    Sha256Sum256Ret r = sha256_sum256(in);
    memcpy(out, r.a, sizeof r.a);
    return (Int)sizeof r.a;
}

static Int sum224(Slice in, Byte *out) {
    Sha256Sum224Ret r = sha256_sum224(in);
    memcpy(out, r.a, sizeof r.a);
    return (Int)sizeof r.a;
}

static void TestGolden(TestingT *t) {
    testdigest_golden(t, "sha256", sum256, sha256_new, golden256, NGOLD(golden256));
    testdigest_golden(t, "sha224", sum224, sha256_new224, golden224, NGOLD(golden224));
}

static void TestLarge(TestingT *t) {
    /* sha256sum of "0123456789" * 1000 */
    testdigest_large(
        t, "sha256 TestLarge", sha256_new, 10000,
        "4c207598af7a20db0e3334dd044399a40e467cb81b37f7ba05a4f76dcbd8fd59");
}

static void TestExtraLarge(TestingT *t) {
    /* sha256sum of "0123456789" * 10000 */
    testdigest_large(
        t, "sha256 TestExtraLarge", sha256_new, 100000,
        "aca9e593cc629cbaa94cd5a07dc029424aad93e5129e5d11f8dcd2f139c16cc0");
}

static void TestSize(TestingT *t) {
    testdigest_sizes(t, "New", sha256_new, SHA256_SIZE, SHA256_BLOCK_SIZE);
    testdigest_sizes(t, "New224", sha256_new224, SHA256_SIZE224, SHA256_BLOCK_SIZE);
}

static void TestHash(TestingT *t) {
    static const char *names[] = {"SHA-224", "SHA-256"};
    TesthashMake fns[] = {sha256_new224, sha256_new};
...
static void BenchmarkHash8Bytes(TestingB *b) {
    testdigest_bench(b, sha256_new, 8);
}

static void BenchmarkHash1K(TestingB *b) {
    testdigest_bench(b, sha256_new, 1024);
}

static void BenchmarkHash8K(TestingB *b) {
    testdigest_bench(b, sha256_new, 8192);
}
