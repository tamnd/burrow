...
- The four public calls Go has on top of a symbol table are in `burrow/runtime.h`. `runtime_func_for_pc` names one address, `runtime_callers_frames` and `runtime_frames_next` walk a slice of them, `runtime_caller` is the single frame shortcut, and `runtime_stack` writes a whole traceback into a buffer you own. Nothing in any of them allocates or waits for a lock, because the callers are logging frameworks and failing programs and one of those two is having a bad day already.
- The frame counting is Go's, including the part of Go's that is a wart. `runtime_callers` numbers its own frame zero and `runtime_caller` numbers its caller zero, which is a difference of one for no reason anybody can defend, and it is in every piece of Go code that skips frames. Being subtly incompatible with all of it would cost more than the tidiness is worth, so burrow counts the same way. `tests/frames_test.c` pins it down as an equality rather than a guess: asked from one frame, frame two of `runtime_callers` and frame one of `runtime_caller` are the same address, because they are the same call site.
- `runtime_frames_next` subtracts one from an address before looking it up and `runtime_func_for_pc` does not. That looks inconsistent and it is deliberate on both sides. The addresses a walk hands back are return addresses, the instruction after a call, and a call in tail position puts that byte in the next function, so a frame walker has to step back to name the right one. `FuncForPC` takes whatever it is given, which is Go's rule and the thing Go's own documentation warns about.
- `RuntimeFrame.pc` is the address that was in the slice, unchanged. Go decrements it, for reasons Go's source calls historical, and this is the one place burrow does not follow: the undecremented address is what an uncaught panic prints and what addr2line wants, and having two nearly equal addresses in circulation for one frame helps nobody.
- `runtime_stack(buf, true)` is partial and says so. The calling goroutine gets its frames and every other goroutine gets a line with its number and its status. Walking a goroutine that is parked on another thread means recovering a frame pointer out of a saved machine context, which is per architecture work belonging to the scheduler rather than to symbolisation. The numbers and the states are still enough to see a deadlock in, and the output does not change shape when the frames arrive.
- Asking for all of them takes the scheduler lock if it is free and gives up on the other goroutines if it is not, rather than waiting, because the thread that is failing may be the thread holding it. A report that is missing the other goroutines beats a hang on top of a crash.
- There is one printer for a frame now rather than two. `burrow__frame_text` formats the two lines and both `burrow__traceback` and `runtime_stack` call it, so a traceback caught in a buffer is byte for byte the one a panic prints. A line that does not fit is dropped whole rather than cut in the middle of a symbol.
- The main goroutine is number 1. It was number 0, because the atomic that hands out ids answers with the value before the add, and `burrow/sched.h` had been saying for months that zero is not a valid id. Go numbers its main goroutine 1 and that number is in every Go traceback anybody has ever read.
